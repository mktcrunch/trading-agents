...
Use tools to fetch account, positions, technical indicators, recent news, competition context,
and quant head-to-head metrics (`get_performance_metrics` or `competition.quant_head_to_head`).
Quant sign rule: raw `comparison.*` is Internal − Baseline. Read `for_you` /
`quant_head_to_head.for_you` (positive = favorable to you). Never treat positive
`comparison.daily_delta_pct` as a Baseline win.
...
- `get_performance_metrics(hours=720, perspective="baseline")` for your-desk quant view
  (`for_you`: positive favors Baseline). Raw `comparison.*` is Internal − Baseline.
