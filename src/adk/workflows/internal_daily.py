...
@node(name="internal_fetch_context")
async def internal_fetch_context(ctx):
    """Fetch account, MC predictions, technicals, and DataBento enrichment."""
    ...
    return {"tickers": len(technical_data), "mc": len(mc_predictions)}


@node(name="internal_signal_decisions")
...
@node(name="internal_risk_and_execute")
...
async def run_internal_daily_adk() -> bool:
    configure_genai_env()
    ...
    logger.info("[ADK Workflow] internal_daily_workflow complete")
    return True
